"""
plot_external_cohort.py — Combined publication-ready figure for §6 External Cohorts.

Reads the four result CSVs produced by run_external_eicu.py and run_external_mimic.py
and produces a single 3-panel figure matching the paper's matplotlib style:

  Panel A  eICU 24h label — AUROC by pipeline × model
  Panel B  eICU 48h label — AUROC by pipeline × model
  Panel C  MIMIC mortality — AUROC by pipeline × model + ΔAUC inset

All colour conventions are identical to the synthetic benchmark figures:
  #2C7BB6  clean (blue)
  #FDAE61  leaky location proxy (orange)
  #D7191C  leaky definitional (red)
  #1A1A1A  oracle (near-black)

Usage
-----
    python src/plot_external_cohort.py \\
        --eicu24   results/tables/external_eicu_24h_results.csv \\
        --eicu48   results/tables/external_eicu_48h_results.csv \\
        --mimic    results/tables/external_mimic_results.csv \\
...
# ─────────────────────────────────────────────────────────────────────────────
# Style constants  (mirror src/plotting.py)
# ─────────────────────────────────────────────────────────────────────────────

PALETTE = {
    "clean":                    "#2C7BB6",
    "leaky_discharge_location": "#D7191C",
    "leaky_definitional":       "#D7191C",   # same red for both leaky types
    "oracle":                   "#1A1A1A",
}
PIPELINE_LABELS = {
    "clean":                    "Clean",
...
def build_figure(
    ...
    # ── Legend patches for pipeline colours ──────────────────────────────
    legend_handles = [
...
# ─────────────────────────────────────────────────────────────────────────────
# Supplementary: per-cohort AUROC × PR-AUC scatter (optional)
# ─────────────────────────────────────────────────────────────────────────────
