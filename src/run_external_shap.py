"""run_external_shap.py — SHAP attribution analysis for external real-world leakage audits (eICU 24h)."""
...
def compute_shap_values_rf(
    ...
    """Compute SHAP values for a fitted RandomForest using TreeExplainer. Returns (n_samples, n_features) for positive class."""
    if not HAS_SHAP:
...
def compute_adi(
    ...
    """ADI(f) = rank_leaky(f) − rank_clean(f). Positive → feature rises in leaky model."""
...
def plot_adi_chart(df_adi: pd.DataFrame, leaky_only: pd.DataFrame, out_path: Path) -> None:
    """Horizontal bar chart of ADI values for shared features, with leaky-only features in a separate panel."""
...
def run_shap_analysis(
    ...
    """Run SHAP analysis comparing clean vs leaky_definitional pipelines on eICU 24h. Returns ADI DataFrame."""
    ...
    # ADI for features present in BOTH pipelines
