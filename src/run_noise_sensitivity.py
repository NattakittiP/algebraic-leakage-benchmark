"""
run_noise_sensitivity.py — Noise sensitivity analysis (Plan item 37).

Tests how TCR residual noise level affects:
  - Clean pipeline AUROC (should stay ~0.50 as noise increases in null scenario)
  - Leaky pipeline AUROC (should stay high — leakage is noise-resistant)
  - AUC inflation (leaky - clean) across noise regimes

Noise levels (TCR SD multipliers relative to baseline σ = 18.6):
  low        : σ × 0.5  =  9.3   (very clean signal)
  baseline   : σ × 1.0  = 18.6  (reference from null scenario)
  medium     : σ × 1.5  = 27.9
  high       : σ × 2.0  = 37.2
  very_high  : σ × 3.0  = 55.8  (extreme noise)

Key finding to report:
  "Definitional leakage remained harmful across noise regimes,
   although absolute inflation varied with outcome stochasticity."
...
# ---------------------------------------------------------------------------
# Helpers: one-fold evaluation (clean and leaky)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# One experiment: fixed noise level × one seed
# ---------------------------------------------------------------------------
...
def run_one_noise_seed(
    ...
    Parameters
    ----------
    fold_pbar : optional tqdm bar passed in from caller to display fold progress.
    """
    ...
    for fold_i, (train_idx, test_idx) in enumerate(
        ...
        # Update external fold pbar if provided
...
def main():
    ...
    total_folds = total_runs * args.outer_folds   # clean + leaky share same splits
    ...
    # ── Overall progress bar (counts individual seed-level runs) ──────────────
    overall_pbar = tqdm(
        total=total_runs,
        desc="Overall",
        ncols=90,
