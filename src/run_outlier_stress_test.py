"""
run_outlier_stress_test.py — Outlier / plausibility stress test (Plan item 39).

Tests how physiologically implausible values affect model performance
and how different cleaning strategies compare.

Implausible value types injected:
  neg_tg4h     : TG4h < 0  (negative — physiologically impossible)
  extreme_tg4h : TG4h > 1500 mg/dL (extreme high)
  extreme_hct  : Hct < 20% or > 65% (extreme haematocrit)
  extreme_tp   : TP < 4.0 or > 10.0 g/dL (extreme total protein)

Contamination rates: 0% (baseline), 1%, 5%

Cleaning strategies:
  no_cleaning        : retain implausible values unchanged
  global_cleaning    : clip/remove based on global plausibility rules before CV
  fold_cleaning      : identify implausible rows in training fold only
  prespecified_clean : apply pre-registered plausibility bounds (best practice)

Key finding to report:
  "Pre-specified fold-level plausibility cleaning was the most robust strategy,
   while no-cleaning degraded performance at ≥5% contamination rates."
...
# ---------------------------------------------------------------------------
# Plausibility bounds (pre-specified before any model run)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# One experiment: fixed outlier scenario × strategy × one seed
# ---------------------------------------------------------------------------
...
def main():
    ...
    # 6. Plot: outlier_stress_heatmap.png
    ...
    # 7. Plot: outlier_strategy_comparison.png
    ...
    # 8. Plot: outlier_contamination_rate.png
