"""Primary synthetic data generator for paired TCR/TG4h biomarker datasets."""
...
def compute_wbv(hct: np.ndarray, tp: np.ndarray) -> np.ndarray:
    """Whole Blood Viscosity (mPa·s) by de Simone formula: WBV = 0.12 × Hct + 0.17 × (TP − 2.07)."""
...
def generate_tcr_null(
    ...
    **kwargs,
    ...
    """Null scenario: TCR is independent of all predictors (expected AUROC ≈ 0.48–0.52)."""
...
def generate_tcr_weak_signal(
    ...
    **kwargs,
    ...
    """Weak signal: age, HDL, BMI have small effects on TCR (expected AUROC ≈ 0.55–0.60)."""
    tcr_mu = cfg.get("tcr_mu", 52.2)
    tcr_sd = cfg.get("tcr_sd", 18.6)
    tcr_low = cfg.get("tcr_low", -10.0)
    tcr_high = cfg.get("tcr_high", 99.9)
...
def generate_tcr_moderate_signal(
    ...
    **kwargs,
    ...
    """Moderate signal: TG0h and BMI predict TCR (expected AUROC ≈ 0.65–0.75)."""
    tcr_mu = cfg.get("tcr_mu", 52.2)
    tcr_sd = cfg.get("tcr_sd", 18.6)
    tcr_low = cfg.get("tcr_low", -10.0)
    tcr_high = cfg.get("tcr_high", 99.9)
    ...
    noise = rng.normal(0, tcr_sd * 0.70, size=n)
    tcr = np.clip(mu_i + noise, tcr_low, tcr_high)
    return tcr
...
def generate_tcr_wbv_positive(
    ...
    **kwargs,
    ...
    """WBV positive-control: WBV has a real, detectable effect on TCR (expected AUROC ≈ 0.83–0.87)."""
    tcr_mu = cfg.get("tcr_mu", 52.2)
    tcr_sd = cfg.get("tcr_sd", 18.6)
    tcr_low = cfg.get("tcr_low", -10.0)
    tcr_high = cfg.get("tcr_high", 99.9)

    beta_wbv = cfg.get("beta_wbv", 7.0)

    wbv_z = (wbv - wbv.mean()) / (wbv.std() + 1e-8)

    mu_i = tcr_mu + beta_wbv * wbv_z
    noise = rng.normal(0, tcr_sd * 0.40, size=n)
    tcr = np.clip(mu_i + noise, tcr_low, tcr_high)
    return tcr
    ...
    df = pd.DataFrame(
        {
            ...
            # TCR and TG4h included for leakage experiments; exclude from clean predictors.
            "tcr": np.round(tcr, 2),
...
def main():
    ...
    for step in tqdm(save_steps, desc="Saving outputs", ncols=70, colour="green"):
        if step == "saving CSV":
            ...
            n_saved = sum(1 for _ in open(out_path)) - 1
            if n_saved != n:
