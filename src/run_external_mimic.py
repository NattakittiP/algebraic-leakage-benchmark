...
class FoldSealedPreprocessorMIMIC:
    """Fold-sealed preprocessing: median/mode imputation, OrdinalEncoder, Winsoriser, StandardScaler."""
...
def oracle_auroc(df: pd.DataFrame, label_col: str) -> dict:
    """Oracle score = 1[discharge_location == 'DIED']. Agreement with label_mortality ≈ 99.92 %."""
...
def run_cv_pipeline(
    ...
    """Repeated stratified k-fold CV for one pipeline. SMOTE applied inside each training fold."""
