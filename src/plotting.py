...
Figure 1: DAG (data-generating mechanism)
Figure 2: Synthetic cohort validation (distributions + correlation heatmap)
Figure 3: Clean vs leaky AUROC/PR-AUC/Brier comparison (MAIN figure)
Figure 4: Feature attribution distortion (SHAP beeswarm clean vs leaky)
Figure 5: Scenario sensitivity (4 scenarios × seeds)
Figure 6: Calibration comparison (clean vs leaky vs domain shift)
...
PALETTE_CLEAN  = "#2C7BB6"    # blue
PALETTE_LEAKY  = "#D7191C"    # red
PALETTE_MEDIUM = "#FDAE61"    # orange
PALETTE_WBV    = "#1A9641"    # green
...
def figure1_dag(out_path: Path):
    ...
    arrows = [
        ...
        ("TG0h", "TCR"),
        ("HDL/LDL", "TCR"),
        ("TCR", "TG4h"),        # DERIVE TG4h FROM TCR
        ("TCR", "low_TCR"),     # label derived from TCR
        ("TG0h", "TG4h"),       # TG4h = TG0h × (1 - TCR/100)
    ]
...
def figure2_cohort_validation(df: pd.DataFrame, out_path: Path):
    ...
    # Correlation heatmap in the last panel
...
# ---------------------------------------------------------------------------
# Figure 3: Leakage benchmark (MAIN)
# ---------------------------------------------------------------------------
...
def figure3_leakage_benchmark(leakage_csv: Path, out_path: Path):
    ...
    # Use LogisticRegression results only for clarity
...
def figure6_calibration(out_path: Path):
    ...
    # Simulated calibration curves for illustration
...
def main():
    ...
    # Figure 2: Cohort validation (needs null dataset)
