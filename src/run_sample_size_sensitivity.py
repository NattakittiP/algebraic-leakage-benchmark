"""
run_sample_size_sensitivity.py — Sample Size Sensitivity Analysis (Plan item 36).

Tests how sample size affects:
  - Clean pipeline AUROC convergence toward 0.50 (null scenario)
  - AUROC SD (precision / CI width) across sample sizes
  - % seeds in null range [0.45, 0.55] vs n
  - TOST equivalence conclusion vs n
  - Leaky (TG4h) AUROC across sample sizes (should stay ~1.00 regardless of n)
  - Signal detectability across scenarios (weak/moderate/wbv_positive) vs n

Sample sizes: 300, 500, 750, 1500, 3000, 6000, 10000, 50000, 100000
Scenarios:    null, weak_signal, moderate_signal, wbv_positive
Seeds:        30 per (size × scenario)
Pipelines:    clean (all scenarios) + leaky/TG4h (null scenario only)

Key findings expected:
  1. Null AUROC converges to 0.50 as n increases.
  2. AUROC SD narrows monotonically with n (precision improves).
  3. % seeds in null range plateaus at ~95% for n ≥ 750.
  4. Leaky AUROC stays ≥ 0.99 regardless of n — leakage is n-invariant.
  5. Signal scenarios become more detectable (lower SD) at larger n.
...
Usage:
  ...
  python src/run_sample_size_sensitivity.py --quick
"""
...
def aggregate_results(records: list[dict]) -> pd.DataFrame:
    ...
    # Filter out failed runs (rows that have a non-null "error" entry)
    ...
    # Sort: scenario natural order then n ascending
...
def plot_auroc_convergence(summary: pd.DataFrame, out_path: Path):
    ...
    ref_auroc = [0.50, 0.58, 0.67, 0.83]   # rough expected
    ...
    for ax, sc, title, col, ref in zip(axes, scenarios, titles, colors, ref_auroc):
        ...
        if sc == "null":
            ...
            # Overlay leaky AUROC if available
...
def plot_null_quality_vs_n(summary: pd.DataFrame, out_path: Path):
    ...
    # --- Panel 2: AUROC mean ± SD + TOST-like band ---
    ax = axes[1]
    mu = sub["clean_AUROC_mean"].values
...
def main():
    ...
    # ── Task list ─────────────────────────────────────────────────────────────
    # All scenarios × all sizes × seeds (clean)
    # + null scenario × all sizes × seeds (leaky)
    ...
    # =========================================================================
    # PHASE 1 — Clean pipeline (all scenarios × all sizes)
    # =========================================================================
    tqdm.write(f"\n{'─'*70}")
    ...
    # =========================================================================
    # PHASE 2 — Leaky pipeline (null scenario only)
    # =========================================================================
    tqdm.write(f"\n{'─'*70}")
    ...
    if "null" in scenarios:
        ...
        for n_sz in leaky_size_pbar:
            ...
            for seed in leaky_seed_pbar:
                ...
                # Store as separate "null_leaky" record
