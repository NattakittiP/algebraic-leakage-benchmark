"""
run_external_eicu.py — External real-world leakage audit on eICU Collaborative Research Database.

Paper section: §6 External Real-World Leakage Audits — Cohort A

Context
-------
label24h = 1  iff  hospitaldischargestatus == 'Expired'  AND  unitdischargeoffset_num <= 1440
label48h = 1  iff  hospitaldischargestatus == 'Expired'  AND  unitdischargeoffset_num <= 2880

Both labels are ALGEBRAICALLY DEFINED by two variables already present as features.
This is a direct instance of Theorem 2 (General Algebraic Leakage):
    Y = h(A, B),  A = hospitaldischargestatus,  B = unitdischargeoffset_num
    Including A and B in the feature set enables AUROC → 1 (oracle = 1.0000).

Four pipelines are evaluated:
  1. clean          — pre-ICU covariates only (demographics + labs + vitals)
                      excludes ALL discharge/outcome variables
  2. leaky_location — clean + hospitaldischargelocation + unitdischargelocation
                      (post-outcome admin fields that proxy death)
  3. leaky_defn     — clean + hospitaldischargestatus + unitdischargeoffset_num
                      (exact definitional antecedents of the label formula)
  4. oracle         — deterministic rule score (no learning); AUROC = theoretical ceiling

Design matches the synthetic benchmark:
  ...
  - Primary metric: AUROC; secondary: PR-AUC (important given class imbalance ~2.8 %)
...
NUMERIC_CLEAN = [
    "age_num",
    # Labs (aggregated over ICU stay, pre-outcome window)
...
# Categorical features encoding admission context (all pre-outcome)
CATEGORICAL_CLEAN = [
    "gender",           # Female / Male / Other / Unknown
    "ethnicity",        # 6 categories
    "unittype",         # 8 ICU unit types
    "unitstaytype",     # admit / transfer / ...
    "hospitaladmitsource",
    "unitadmitsource",
]
...
# Leaky pipeline A: post-discharge LOCATION variables (proxy for death)
LEAKY_LOCATION_NUMERIC: list[str] = []
LEAKY_LOCATION_CATEGORICAL = [
    "hospitaldischargelocation",   # includes "Death"
    "unitdischargelocation",       # includes "Death"
]

# Leaky pipeline B: exact DEFINITIONAL formula components
# label24h = (hospitaldischargestatus == 'Expired') AND (unitdischargeoffset_num <= 1440)
LEAKY_DEFN_NUMERIC = ["unitdischargeoffset_num"]
LEAKY_DEFN_CATEGORICAL = ["hospitaldischargestatus"]   # Expired / Alive

# Oracle threshold (minutes)
ORACLE_THRESHOLD_24H = 1440   # 24 × 60
ORACLE_THRESHOLD_48H = 2880   # 48 × 60


# ─────────────────────────────────────────────────────────────────────────────
# Fold-sealed preprocessor for external cohort
# ─────────────────────────────────────────────────────────────────────────────
...
def build_models() -> dict:
    ...
    if HAS_XGB:
        # Compute scale_pos_weight from overall dataset prevalence (~2.8 %)
        # Will be overridden per-fold inside the CV loop
...
# ─────────────────────────────────────────────────────────────────────────────
# Oracle AUROC (no learning required)
# ─────────────────────────────────────────────────────────────────────────────
...
def oracle_auroc(df: pd.DataFrame, label_col: str, offset_threshold: int) -> dict:
    ...
    # If oracle is perfect (100 % agreement), AUROC = 1.0 exactly
    try:
...
def run_cv_pipeline(
    ...
    for seed in seed_pbar:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(
            ...
            # ── SMOTE inside training fold only ───────────────────────────
            ...
            for model_name, model in models.items():
                ...
                # XGBoost: set per-fold class weight
...
def run_label(
    ...
    # Save raw seed-level records for reproducibility
